...
class   Calldata {
	...
	public function readAll() {				
		$sth = $this->dbh->prepare("SELECT * FROM calldata");
		$sth->execute();
		return json_encode($sth->fetchAll(PDO::FETCH_ASSOC));
	}

	public function view_readAll() {				
		$sth = $this->dbh->prepare("SELECT * FROM v_calldata");
		$sth->execute();
		return json_encode($sth->fetchAll(PDO::FETCH_ASSOC));
	}

	public function read($obj) {
		$sth = $this->dbh->prepare("SELECT * FROM calldata WHERE ID=?");
	...
	public function create($obj) {		
		$sth = $this->dbh->prepare("INSERT INTO calldata (ID,nom,prenom,tel,email,rang,experiance,region,pays,profil,ID_type_prestataire,ID_diplome,ID_ville,ID_langue,ID_categorie,ID_sous_categorie,ID_utilisateur,ID_statut,ID_disponibilite,date_creation) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);");
	...
	public function delete($obj) {				
		$sth = $this->dbh->prepare("DELETE FROM calldata WHERE ID=?");
		$sth->execute(array($obj->ID));
		return json_encode(1);
	}
}
